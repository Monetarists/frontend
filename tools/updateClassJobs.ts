...
export async function updateClassJobs(xiv: any) {
	...
	try {
		...
		let columns = 'ID,Abbreviation,Name_en,Name_de,Name_fr,Name_ja,ClassJobCategoryTargetID,DohDolJobIndex';
		...
		try {
			...
			fs.writeFileSync('data/ClassJob.json', JSON.stringify(data.filter((classJob) => classJob.ClassJobCategoryTargetID === 33)));
		} catch (error) {
