...
const SidebarCrafter = ({ classJob, children, ...rest }: SidebarCrafterProps) => {
	const [settings] = useSettings();
	const [jobName, setNewJobName] = useState(classJob.Name_en);

	useEffect(() => {
		switch (settings.monetarist_language) {
			case 'de': setNewJobName(classJob.Name_de); break;
			case 'fr': setNewJobName(classJob.Name_fr); break;
			case 'ja': setNewJobName(classJob.Name_ja); break;
		}
	}, [settings, setNewJobName, classJob]);

	return (
		<>
			<Link href={`/crafter/${classJob.Abbreviation}`} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
				<Flex
					align="center"
					p="1"
					borderRadius="lg"
